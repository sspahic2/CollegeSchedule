let Tabela = (function() {
    ...
    var ubaciAktivnost = function(pocetakSati, vrijemePocetak, tip, naziv, vrijemeKraj, red, duz) {
        for(let i = 1; i < duz; i++) {
            if(pocetakSati == vrijemePocetak) {
                let celija = red[i];
    ...
    var skontajPocetak = function(tabela) {
        let pocetak = -1;
    ...
    var indeksReda = function(dan, tabela) {
        var indeks = -1;
    ...
    var provjeriCjelobrojnost = function (broj1, broj2) {
        return !(Number.isInteger(broj1) && Number.isInteger(broj2))
    }

    var skontajDuzinu = function (red, tabela) {
        let velicina = 0;
